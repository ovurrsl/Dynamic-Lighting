...
test('a non-JSON body is rejected rather than crashing the handler', async (t) => {
  const { app } = await harness()
  t.after(() => app.close())
...
test('preset ids are validated so they cannot escape their licence scope', async (t) => {
  const { app } = await harness()
  t.after(() => app.close())

  const activated = await activate(app, 'AF-OK-0001', 'fingerprint-pppppppppppp')
  const auth = { authorization: `Bearer ${activated.json().token}` }

  const response = await app.inject({
    method: 'PUT',
    url: '/v1/presets/has%20a%20space',
    headers: auth,
...
test('an unknown API path returns JSON, not the SPA fallback', async (t) => {
  const { app } = await harness()
  t.after(() => app.close())
...
test('an unknown update channel is rejected by the schema', async (t) => {
  const { app } = await harness()
  t.after(() => app.close())

  const response = await app.inject({ method: 'GET', url: '/v1/updates/manifest?channel=nonsense' })
  assert.equal(response.statusCode, 400)
})
